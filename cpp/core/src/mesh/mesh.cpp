...
void Mesh<vertex_dim, element_dim>::Initialize(const std::string& binary_file_name) {
    ...
    // Assume all elements have the same volume, we we compute for the first element.
    Eigen::Matrix<real, vertex_dim, element_dim> element;
    for (int k = 0; k < element_dim; ++k) {
        element.col(k) = vertices.col(elements(k, 0));
    }
    VectorXr maxCoord = element.rowwise().maxCoeff();
    VectorXr minCoord = element.rowwise().minCoeff();
    dx_ = (maxCoord - minCoord)[0];
    dy_ = (maxCoord - minCoord)[1];
    dz_ = vertex_dim == 3 ? (maxCoord - minCoord)[2] : ToReal(0);
}
...
template<>
const std::array<real, 2> Mesh<2, 3>::dx() const {
    CheckError(false, "dx is not defined for triangle meshes.");
    return {{0, 0}};
}

template<>
const std::array<real, 2> Mesh<2, 4>::dx() const {
    return {{dx_, dy_}};
}

template<>
const std::array<real, 3> Mesh<3, 4>::dx() const {
    CheckError(false, "dx is not defined for tet meshes.");
    return {{0, 0, 0}};
}

template<>
const std::array<real, 3> Mesh<3, 8>::dx() const {
    return {{dx_, dy_, dz_}};
}
