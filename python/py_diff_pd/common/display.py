...
def display_hex_mesh(hex_mesh, xlim=None, ylim=None, zlim=None, title=None, file_name=None, show=True):
    ...
    for e in range(element_num):
        ...
        lines = [(0, 1), (1, 3), (3, 2), (2, 0),
            ...
            (0, 4), (1, 5), (2, 6), (3, 7)]
        for i, j in lines:
            ax.plot([v[i, 0], v[j, 0]], [v[i, 1], v[j, 1]], [v[i, 2], v[j, 2]], color='tab:red', linewidth=0.5, alpha=0.5)

    paddingRatio = 0.2
    x_min -= paddingRatio*(x_max-x_min)
    x_max += paddingRatio*(x_max-x_min)
    y_min -= paddingRatio*(y_max-y_min)
    y_max += paddingRatio*(y_max-y_min)
    z_min -= paddingRatio*(z_max-z_min)
    z_max += paddingRatio*(z_max-z_min)
    maxRange = max(x_max-x_min, y_max-y_min, z_max-z_min)
    ax.set_xlabel('x')
    ax.set_ylabel('y')
    ax.set_zlabel('z')
    if xlim is None:
        ax.set_xlim([(x_min+x_max)/2-maxRange/2, (x_min+x_max)/2+maxRange/2])
    else:
        ax.set_xlim(xlim)
    if ylim is None:
        ax.set_ylim([(y_min+y_max)/2-maxRange/2, (y_min+y_max)/2+maxRange/2])
    else:
        ax.set_ylim(ylim)
    if zlim is None:
        ax.set_zlim([(z_min+z_max)/2-maxRange/2, (z_min+z_max)/2+maxRange/2])
    else:
        ax.set_zlim(zlim)
    # ax.set_xticks([])
    # ax.set_yticks([])
