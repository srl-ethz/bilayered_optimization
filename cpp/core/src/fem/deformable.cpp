...
void Deformable<vertex_dim, element_dim>::Forward(const std::string& method, const VectorXr& q, const VectorXr& v,
    ...
    if (BeginsWith(method, "pd")) ForwardProjectiveDynamics(method, q, v, a, f_ext, dt, options, q_next, v_next, active_contact_idx);
    // else if (method == "semi_implicit") ForwardSemiImplicit(q, v, a, f_ext, dt, options, q_next, v_next, active_contact_idx);
...
void Deformable<vertex_dim, element_dim>::Backward(const std::string& method, const VectorXr& q, const VectorXr& v, const VectorXr& a,
    ...
    std::vector<VectorXr>& dl_dw, VectorXr& dl_dstate_p) const {
    if (BeginsWith(method, "pd"))
        BackwardProjectiveDynamics(method, q, v, a, f_ext, dt, q_next, v_next, active_contact_idx, dl_dq_next, dl_dv_next, options,
            dl_dq, dl_dv, dl_da, dl_df_ext, dl_dw, dl_dstate_p);
    // else if (method == "semi_implicit")
...
void Deformable<vertex_dim, element_dim>::PyBackward(const std::string& method, const std::vector<real>& q, const std::vector<real>& v,
    ...
    std::vector<real>& dl_dw, std::vector<real>& dl_dstate_p) const {
    VectorXr dl_dq_eig, dl_dv_eig, dl_da_eig, dl_df_ext_eig, dl_dstate_p_eig;
    std::vector<VectorXr> dl_dw_eig_vector(NumOfPdElementEnergies());
    Backward(method, ToEigenVector(q), ToEigenVector(v), ToEigenVector(a), ToEigenVector(f_ext), dt, ToEigenVector(q_next),
        ...
        dl_dq_eig, dl_dv_eig, dl_da_eig, dl_df_ext_eig, dl_dw_eig_vector, dl_dstate_p_eig);
    ...
    dl_dw.clear();
    for (auto& dl_dw_eig : dl_dw_eig_vector) {
        const std::vector<real> dl_dw_eig_single = ToStdVector(dl_dw_eig);
        dl_dw.insert(dl_dw.end(), dl_dw_eig_single.begin(), dl_dw_eig_single.end());
    }
}
