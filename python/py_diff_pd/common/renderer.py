    ...
    def add_hex_mesh(self, hex_mesh, transforms=None, render_voxel_edge=False, texture_map=None,
        ...
        if render_voxel_edge:
            if texture_img is None:
                texture_img = Path(root_path) / 'asset/texture/grid_thin.png'
    ...
    def add_tri_mesh(self, tri_mesh, transforms=None, render_tet_edge=False, color=(.5, .5, .5), texture_img=None):
        ...
        if render_tet_edge:
            if texture_img is None:
                ...
                texture_img = Path(root_path) / 'asset/texture/tri_grid_thin.png'   # The outer edges will always be double, so should be thinner
                # You can use the code below to create a grid.png.
                # edge_width = 4
                # img_size = 64
                # texture_img = self.__temporary_folder / 'hex_{:08d}_texture.png'.format(hex_num)
