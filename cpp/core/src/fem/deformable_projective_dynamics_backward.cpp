    ...
    for (int i = 0; i < element_num; ++i) {
        ...
        for (int j = 0; j < sample_num; ++j) {
            ...
            for (const auto& energy : pd_element_energies_) {
                const real w = energy->stiffness(i) * element_volume_ / sample_num;
...
void Deformable<vertex_dim, element_dim>::BackwardProjectiveDynamics(const std::string& method, const VectorXr& q, const VectorXr& v,
    ...
    const int element_num = mesh_.NumOfElements();
    for (int i = 0; i < w_dofs; ++i) dl_dw[i] = VectorXr::Zero(element_num);
    ...
    std::vector<SparseMatrixElements> nonzeros_mat_w(w_dofs);
    PdEnergyForceDifferential(q_next, false, true, use_precomputed_data, nonzeros_q, nonzeros_mat_w);
    for (int i = 0; i < w_dofs; ++i) dl_dw[i] += VectorSparseMatrixProduct(adjoint, dofs_, element_num, nonzeros_mat_w[i]);
    // Equivalent code:
