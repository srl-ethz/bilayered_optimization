...
public:
    ...
    const int NumOfElements() const {
        return static_cast<int>(mesh_.NumOfElements());
    }
    ...
    void AddPdEnergyHeterogeneous(const std::string& energy_type, const VectorXr& params);
    const real ComputePdEnergy(const VectorXr& q, const bool use_precomputed_data) const;
    const VectorXr ComputePdElementEnergy(const VectorXr& q, const bool use_precomputed_data) const;
    const VectorXr PdEnergyForce(const VectorXr& q, const bool use_precomputed_data) const;
    const VectorXr PdEnergyForceDifferential(const VectorXr& q, const VectorXr& dq, const std::vector<VectorXr>& dw) const;
    void PdEnergyForceDifferential(const VectorXr& q, const bool require_dq, const bool require_dw,
        const bool use_precomputed_data, SparseMatrixElements& dq, std::vector<SparseMatrixElements>& dw) const;
    ...
    void PyAddPdEnergyHeterogeneous(const std::string& energy_type, const std::vector<real>& params);
