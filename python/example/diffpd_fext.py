# -----------------------------------------------------------------------------
# Example of a 3D beam with Dirichlet boundary conditions falling under gravity
# -----------------------------------------------------------------------------
...
def main (num_frames, dt, visualization):
    ...
    q, v = beam.q0, beam.v0
    force_per_volume = 1 / 0.003    # N/m^3
    f_ext = np.zeros(beam.dofs).reshape(-1,3)
    #f_ext[2::3] += 1 * 0.003    # Volume of whole structure is 0.003m^3
    elements_per_vertex = np.zeros(beam.dofs//3).reshape(-1,1)
    for i in range(beam.mesh.NumOfElements()):
        for v_idx in beam.mesh.py_element(i):
            f_ext[v_idx,2] += beam.mesh.element_volume(i) * force_per_volume
    ...
    for i in range(num_frames):
        ...
        com.append(q.view(-1,3).detach().cpu().numpy().mean(0))
