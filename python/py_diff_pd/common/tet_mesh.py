...
from py_diff_pd.common.common import ndarray, filter_unused_vertices, print_warning
...
def generate_tet_mesh(vertices, faces, bin_file_name):
    with open(bin_file_name, 'wb') as f:
        ...
        # Faces.
        faces = ndarray(faces).astype(int)
...
def tetrahedralize(triangle_mesh_file_name, visualize=False, normalize_input=True, options=None):
    ...
    # Make manifold often unnecesarily removes too many edges.
    #tet.make_manifold()

    # from pymeshfix import _meshfix

    # tin = _meshfix.PyTMesh()
    # tin.load_array(tet.v, tet.f)
    # f_inter = tin.select_intersecting_triangles(justproper=False)
    
    # if f_inter.shape[0] != 0:
    #     print_warning("Self-intersecting edges were found with pymeshfix, this may cause incorrect results after tetrahedralization. Please make sure mesh is manifold, then safely disregard this warning.")
