...
def generate_hexmesh_from_voxel(binary_mask, bin_file_name, dx=1, dy=None, dz=None, origin=np.zeros(3)):
    """
    Upper left corner of image is the origin. Contains a lot of vertices doubled.
    
    binary_mask (shape [H, W, D])
    """
    if dy is None:
        dy = dx
    if dz is None:
    ...
    with open(bin_file_name, 'wb') as f:
        ...
        vert_y = []
        vert_z = []
        elements = []  # Ordering is to be 0 lower left, 1 upper left, 2 lower right, 3 upper right.
        counts_ele = 0
        for i in range(binary_mask.shape[0]):
            for j in range(binary_mask.shape[1]):
                for k in range(binary_mask.shape[2]):
                    if binary_mask[i,j,k] == 1:
                        order = vertex_order(i,j,k)
                        for vi, vj, vk in order:
                            vx = origin[0] + vi*dx
                            vy = origin[1] + vj*dy
                            vz = origin[2] + vk*dz

                            if (vx, vy, vz) in vertex_set:
                                # Find previous match in vertices
                                idx = np.where(np.logical_and(np.logical_and(np.array(vert_x)==vx, np.array(vert_y)==vy), np.array(vert_z)==vz))[0]
                                assert len(idx) == 1, f"For duplicate vertex, {len(idx)} matches were found. Should only be one."
                                # Append this vertex as the element corner
                                elements.append(idx[0])
                            else:
                                vertex_set.add((vx, vy, vz))
                                vert_x.append(vx)
                                vert_y.append(vy)
                                vert_z.append(vz)
                                elements.append(counts_ele)
                                counts_ele += 1

        elements = np.array(elements).reshape(-1,8)

        # Now we write all of these in order
        # Vertices.
        f.write(struct.pack('i', 3))
        f.write(struct.pack('i', len(vert_x)))
