...
void Deformable<vertex_dim, element_dim>::AddPdEnergy(const std::string& energy_type, const std::vector<real>& params,
    ...
    if (energy_type == "planar_collision") {
        ...
        const VectorXr stiffness = VectorXr::Constant(mesh_.NumOfElements(), params[0]);
...
template<int vertex_dim, int element_dim>
void Deformable<vertex_dim, element_dim>::AddPdEnergyHeterogeneous(const std::string& energy_type, const VectorXr& params) {
    pd_solver_ready_ = false;
    // const int param_size = static_cast<int>(params.size());
    if (energy_type == "corotated") {
        auto energy = std::make_shared<CorotatedPdElementEnergy<vertex_dim>>();

        CheckError(params.size() == mesh_.NumOfElements(), "Inconsistent param size.");
        energy->Initialize(params);
        ...
        for (int i = 0; i < element_num; ++i) {
            ...
            for (int j = 0; j < sample_num; ++j) {
                ...
                if (use_precomputed_data)
                    element_energy[i] += energy->EnergyDensity(i, F_auxiliary_[i][j],
                        projections_[energy_cnt][i][j]) * element_volume_ / sample_num;
                else
                    element_energy[i] += energy->EnergyDensity(i, F) * element_volume_ / sample_num;
            }
        }
        ++energy_cnt;
    }
...
template<int vertex_dim, int element_dim>
const VectorXr Deformable<vertex_dim, element_dim>::ComputePdElementEnergy(const VectorXr& q, const bool use_precomputed_data) const {
    const int element_num = mesh_.NumOfElements();
    const int sample_num = GetNumOfSamplesInElement();
    VectorXr element_energy = VectorXr::Zero(element_num);
    int energy_cnt = 0;
    for (const auto& energy : pd_element_energies_) {
        #pragma omp parallel for
        for (int i = 0; i < element_num; ++i) {
            ...
            for (int j = 0; j < sample_num; ++j) {
                if (use_precomputed_data)
                    ...
                    element_energy(i) += energy->EnergyDensity(
                        i, DeformationGradient(i, deformed, j)) * element_volume_ / sample_num;
            }
        }
        ++energy_cnt;
    }
    ...
    for (const auto& energy : pd_element_energies_) {
        ...
        for (int i = 0; i < element_num; ++i) {
            ...
            for (int j = 0; j < sample_num; ++j) {
                ...
                if (use_precomputed_data)
                    P = energy->StressTensor(i, F_auxiliary_[i][j], projections_[energy_cnt][i][j]);
                else
                    P = energy->StressTensor(i, F);
...
const VectorXr Deformable<vertex_dim, element_dim>::PdEnergyForceDifferential(const VectorXr& q, const VectorXr& dq,
    const std::vector<VectorXr>& dw) const {
    ...
    for (const auto& energy : pd_element_energies_) {
        ...
        for (int i = 0; i < element_num; ++i) {
            ...
            const real inv_w = 1 / energy->stiffness(i);
            const real dw_energy_element = dw[energy_cnt][i];
            for (int j = 0; j < sample_num; ++j) {
                const auto F = DeformationGradient(i, deformed, j);
                const auto dF = DeformationGradient(i, ddeformed, j);
                // We use the assumption that stress tensor is linear to w.
                const Eigen::Matrix<real, vertex_dim, vertex_dim> dP = energy->StressTensorDifferential(i, F, dF)
                    + energy->StressTensor(i, F) * inv_w * dw_energy_element;
...
void Deformable<vertex_dim, element_dim>::PdEnergyForceDifferential(const VectorXr& q, const bool require_dq,
    const bool require_dw, const bool use_precomputed_data, SparseMatrixElements& dq, std::vector<SparseMatrixElements>& dw) const {
    dq.clear();
    for (auto& dw_energy : dw) dw_energy.clear();
    const int w_dofs = NumOfPdElementEnergies();
        ...
        for (const auto& energy : pd_element_energies_) {
            for (int i = 0; i < element_num; ++i) {
                ...
                for (int j = 0; j < sample_num; ++j) {
                    ...
                    if (use_precomputed_data)
                        dP_from_dF = energy->StressTensorDifferential(i, F_auxiliary_[i][j], projections_[energy_cnt][i][j]) * dF;
                    else
                        dP_from_dF = energy->StressTensorDifferential(i, DeformationGradient(i, deformed, j)) * dF;
    ...
    if (require_dw) {
        for (int energy_cnt = 0; energy_cnt < w_dofs; ++energy_cnt) {
            dw[energy_cnt].resize(element_num * sample_num * element_dim * vertex_dim);
            const VectorXr inv_w = 1 / pd_element_energies_[energy_cnt]->stiffness().array();
            #pragma omp parallel for
            for (int i = 0; i < element_num; ++i) {
                const Eigen::Matrix<int, element_dim, 1> vi = mesh_.element(i);
                const auto deformed = ScatterToElement(q, i);
                for (int j = 0; j < sample_num; ++j) {
                    ...
                    Eigen::Matrix<real, vertex_dim, vertex_dim> P;
                    if (use_precomputed_data) {
                        P = pd_element_energies_[energy_cnt]->StressTensor(i, F_auxiliary_[i][j], projections_[energy_cnt][i][j]);
                    } else {
                        P = pd_element_energies_[energy_cnt]->StressTensor(i, DeformationGradient(i, deformed, j));
                    }
                    const auto dP_from_dw = Flatten(P) * inv_w[i];
                    ...
                    for (int k = 0; k < element_dim; ++k)
                        for (int d = 0; d < vertex_dim; ++d) {
                            const int idx = i * sample_num * element_dim * vertex_dim
                                + j * element_dim * vertex_dim + k * vertex_dim + d;
                            dw[energy_cnt][idx] = Eigen::Triplet<real>(vertex_dim * vi(k) + d,
                                i, df_kd_from_dw(k * vertex_dim + d));
                        }
                }
            }
        }
...
template<int vertex_dim, int element_dim>
void Deformable<vertex_dim, element_dim>::PyAddPdEnergyHeterogeneous(const std::string& energy_type,
    const std::vector<real>& params) {
    AddPdEnergyHeterogeneous(energy_type, ToEigenVector(params));
}
